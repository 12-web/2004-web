...
import { getRepMoveSSCCReport } from '@store/outMappers/report'
import { FilterComponent } from '@pages/ReportPage/ReportPage'
import { RepMoveSSCCInputs } from 'types/report/report'
import { TcsTable } from './TcsTable/TcsTable'

import styles from '../styles.module.css'
import { WarehousesTable } from './WarehousesTable/WarehousesTable'

type RepMoveSSCCFiltersProps = FilterComponent

export const RepMoveSSCCFilters = ({
  isOpen,
  onToggleFilters,
  onGetClick,
}: RepMoveSSCCFiltersProps) => {
  const onCollapseClick = () => onToggleFilters(!isOpen)

  const methods = useForm<RepMoveSSCCInputs>()
  const { handleSubmit, register, setValue } = methods

  const onSubmit = async (data: RepMoveSSCCInputs) => {
    onGetClick(getRepMoveSSCCReport(data))
  }
