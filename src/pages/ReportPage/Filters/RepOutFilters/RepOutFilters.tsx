...
import { getRepOutReport } from '@store/outMappers/report'
...
type RepOutFiltersProps = FilterComponent

export const RepOutFilters = ({ isOpen, onToggleFilters, onGetClick }: RepOutFiltersProps) => {
  const onCollapseClick = () => onToggleFilters(!isOpen)

  const methods = useForm<RepOutInputs>()
  const { handleSubmit, register, setValue } = methods

  const onSubmit = async (data: RepOutInputs) => {
    onGetClick(getRepOutReport(data))
  }
